...
var rider: PackedScene = preload("res://card/summon_cards/rider_card/rider.tscn")
...
func summon(place_position: Vector2):
	for i in range(1):
		var new_rider = rider.instantiate()
		summon_node.add_child(new_rider)
		new_rider.set_summon_id(player_id)
#		print_debug("Player ID: ", player_id, " Summon:", new_waiter)
		new_rider.global_position = place_position + Vector2(randi_range(-50, 50), randi_range(-50, 50))
